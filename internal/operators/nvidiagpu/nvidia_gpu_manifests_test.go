...
var _ = Describe("Manifest generation", func() {
	...
	It("Generates the required OpenShift manifests", func() {
		...
		Expect(manifests).To(HaveKey("50_nvidia_gpu_namespace.yaml"))
		Expect(manifests).To(HaveKey("50_nvidia_gpu_subscription.yaml"))
		Expect(manifests).To(HaveKey("50_nvidia_gpu_operatorgroup.yaml"))
	})
